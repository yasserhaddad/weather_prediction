...
def generate_predictions_ensemble(config_file, nb_models=5, ensembling=False, swag=False, scale_swag=1.0, 
    ...
    for ep in epochs_range:
        ...
        if ensembling or multiple_swag_realizations:
            pred_median_filename = pred_filename + "_median.zarr"
            pred_mean_filename = pred_filename + "_mean.zarr"

            if aggregate_realizations:
                pred_median_filename = pred_filename + "_aggregate_median.zarr"
                pred_mean_filename = pred_filename + "_aggregate_mean.zarr"
        ...
        for i in range(nb_models):
            ...
            if swag:
                pred_model_filename = pred_model_filename[:-3] + \
                                        '_scale{}.zarr'.format(str(scale_swag).replace('.', ''))
            ...
            for j in range(nb_realizations):
                ...
                if multiple_swag_realizations:
                    pred_model_filename = pred_model_filename[:-3].replace(f'_realization{j}', '') + '_realization{}.zarr'.format(j+1)
                    rmse_realization_filename = datadir + 'metrics/rmse_' + description_model_epoch + '_realization{}.zarr'.format(j+1)
                ...
                try:
                    if not aggregate_realizations:
                        prediction_ds = prediction_ds.expand_dims(dim='member')
                        prediction_ds['member'] = [i + j + 1]

                        if i == 0 and j == 0:
                            prediction_ds.to_zarr(pred_filename + '.zarr', encoding={var: {"compressor": compressor} for var in ['z', 't']}, mode='w')
                        else:
                            prediction_ds.to_zarr(pred_filename + '.zarr', append_dim='member')
            ...
            if aggregate_realizations:
                ...
                pred_model_filename_agg = pred_model_filename[:-3] + "_aggregate.zarr"
